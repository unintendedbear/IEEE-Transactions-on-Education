...
In order to deeply study the results of the surveys, we will use the odd ratio measure \cite{bibid}. Therefore, when the students are asked whether they are thinking about studying an engineering or not, the resulting odd ratio is of 7.567 with limits [3.504, 16.342] for boys to study an engineering. This means that in our study case, there is a significant advantage -- more than 7.5 -- of choosing to study particularly an engineering for boys than for girls. Table \ref{tab:oddratios} shows the odd raios that we think better sumarise the results of this study.

\begin{table*}
	\caption[Odd ratios for every influence factor.]{Odd ratios that have been obatined depending on the decision of studying an engineering and the different factors of influence that we try to identify. An \textbf{$*$} indicates the statistically significant values.}
	\label{tab:oddratios}
	
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		Influence factor & Will you study engineering? & Gender & O.R. & Inferior limit & Superior limit \\ \hline
		Classroom environment being mainly male & Yes & Boy & 2.524$*$ & 1.027 & 6.204 \\ \hline
		Classroom environment being mainly male & Yes & Girl & 1.387 & 0.322 & 5.978 \\ \hline
		STEM teachers being mainly male & Yes & Boy & 0.963 & 0.584 & 1.587 \\ \hline
		STEM teachers being mainly male & Yes & Girl & 7.286$*$ & 1.359 & 39.055 \\ \hline
		\end{tabular}
	\end{center}
\end{table*}

In regard of the environment in the classroom, we distinguish between the fellow students and the teachers. This way, Table \ref{tab:oddratios} reports a significant advantage of 2.524 for boys to choose an engineering when the environment in the classroom is of a majority of boys instead of an equal or majority of girls situation. In the case of girls, there is also a slight advantage in a mainly male environment but it is not significant. However, to have mainly, or all, male teachers in STEM subjects does matter for the girls, given that in that case girls present an advantage of 7.286 of choosing an engineering than for the other case where they report having teachers from both genders or mainly female. This not necessarily means that female role models are not important for the girls' decision, but rather that those girls that already feel comfortable in a mainly male environment, are more likely to study engineering than those who do not.
